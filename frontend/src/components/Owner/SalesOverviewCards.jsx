import { DollarSign, TrendingUp, Package, CreditCard, ArrowUp, ArrowDown } from 'lucide-react';

const fmt = (v) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 0 }).format(v);

const cards = [
  { key: 'revenue', label: 'Total Revenue', icon: DollarSign, valueKey: 'total_revenue', changeKey: 'revenue_change', subtitle: (d) => `${d.total_orders?.toLocaleString('en-IN')} orders`, bg: '#EFF6FF', iconBg: '#3B82F6', format: true },
  { key: 'profit', label: 'Total Profit', icon: TrendingUp, valueKey: 'total_profit', changeKey: 'profit_change', subtitle: (d) => `${d.profit_margin}% margin`, bg: '#F0FDF4', iconBg: '#10B981', format: true },
  { key: 'items', label: 'Items Sold', icon: Package, valueKey: 'total_items_sold', changeKey: null, subtitle: (d) => `Across ${d.total_orders?.toLocaleString('en-IN')} orders`, bg: '#FAF5FF', iconBg: '#8B5CF6', format: false },
  { key: 'avg', label: 'Avg Order Value', icon: CreditCard, valueKey: 'avg_order_value', changeKey: null, subtitle: () => 'Target: ₹2,000', bg: '#FFF7ED', iconBg: '#F97316', format: true },
];
        ...
        return (
          <div key={c.key} className="owner-kpi-card" style={{ background: c.bg }}>
            ...
            <div className="owner-kpi-body">
              ...
              <span className="owner-kpi-value">{c.format ? fmt(value) : value?.toLocaleString('en-IN')}</span>
