          ...
          <tbody>
            {sorted.map((p) => (
              <tr key={p.product_id}>
                ...
                <td className="num-cell">{p.total_quantity_sold.toLocaleString('en-IN')}</td>
                <td className="num-cell">{fmt(p.total_revenue)}</td>
                <td className="num-cell">
                  <span>{fmt(p.total_profit)}</span>
                  <span className={`margin-badge ${p.profit_margin >= 25 ? 'green' : p.profit_margin >= 15 ? 'yellow' : 'red'}`}>
                    {p.profit_margin}%
                  </span>
                </td>
                <td>
                  <button className="tpt-view-btn"><Eye size={15} /></button>
                </td>
              </tr>
            ))}
          </tbody>
...
const styles = `
  ...
  .margin-badge { display: inline-block; font-size: 0.68rem; font-weight: 700; padding: 0.1rem 0.4rem; border-radius: 4px; margin-left: 6px; }
  .margin-badge.green { color: #16a34a; background: rgba(22,163,74,0.1); }
  .margin-badge.yellow { color: #ca8a04; background: rgba(202,138,4,0.1); }
  .margin-badge.red { color: #dc2626; background: rgba(220,38,38,0.1); }
  .tpt-view-btn {
