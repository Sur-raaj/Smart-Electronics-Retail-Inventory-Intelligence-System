...
export default function ProductModal({ isOpen, onClose, onSave, product, categories = [], suppliers = [] }) {
  ...
  const blank = { name: '', category_name: '', brand: '', model_number: '', description: '', specifications: '', cost_price: '', selling_price: '', stock_quantity: '', reorder_level: '', supplier_name: '', warranty_months: '', status: 'Active', image_url: '' };
  const [form, setForm] = useState(blank);
  const [errors, setErrors] = useState({});
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (isOpen) setForm(product ? { ...blank, ...product, cost_price: product.cost_price ?? '', selling_price: product.selling_price ?? '', stock_quantity: product.stock_quantity ?? '' } : blank);
    setErrors({});
  }, [isOpen, product]);
  ...
  const validate = () => {
    ...
    if (!form.category_name) e.category_name = 'Required';
    ...
    if (form.stock_quantity === '' || Number(form.stock_quantity) < 0) e.stock_quantity = 'Enter valid quantity';
    setErrors(e);
      ...
      <div className="pm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="pm-body">
          <div className="pm-grid">
            ...
            <Field label="Category" name="category_name" required options={categories.map((c) => c.name)} />
            <Field label="Brand" name="brand" required />
            <Field label="Model Number" name="model_number" />
            <Field label="Cost Price (₹)" name="cost_price" type="number" required />
            <Field label="Selling Price (₹)" name="selling_price" type="number" required />
            <Field label="Stock Quantity" name="stock_quantity" type="number" required />
            <Field label="Reorder Level" name="reorder_level" type="number" />
            <Field label="Supplier" name="supplier_name" options={suppliers.map((s) => s.name)} />
            <Field label="Warranty (months)" name="warranty_months" type="number" />
            <Field label="Status" name="status" options={['Active', 'Discontinued', 'Out of Stock']} />
            <div className="pm-field">
